...
void CChildView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    {
        CRocketPadVisitor launchingSystem;
        CRocketPadTileVisitor landingPad;

        mCity.Accept(&launchingSystem);
        mGrabbedItem->Accept(&landingPad);

        if (landingPad.IsRocketPad())
        {
            if (!launchingSystem.InFlight())
            {
                launchingSystem.LaunchRocket(landingPad);
            }
        }
