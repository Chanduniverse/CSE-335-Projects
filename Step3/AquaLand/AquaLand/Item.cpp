...
* \param filename the file out image is in
*/
...
/**
 * CalcDist Calculates distance between two items
 * \param item2 the item being used for calculating distance
 * \returns distance as a double
 */
double CItem::CalcDist(const CItem& item2)
{
    // Distance in the X and Y directions
    double dx = mX - item2.GetX();
    double dy = mY - item2.GetY();
